...
(async () => {
  ...
  let video, finishDownload, revoked = false;
  const ctx = {
    ...
    fetch: async () => ({ ok: true, headers: { get: () => '8' }, body: { getReader() {
      let step = 0;
      return { read: () => ++step === 1
  ...
  console.log('PASS: complete-download gate, decoded-frame gate, scroll seeking, cleanup, no autoplay');
